--Querying a data warehouse using snowflake schema

--Find average purchase list price and sale price by state
SELECT a.state,
...
--Find the average purchase list price and sale price by store
SELECT s.storename,
       AVG(p.listprice) AS AverageListPrice,
       AVG(p.saleprice) AS AverageSalePrice
    FROM purchase p
...
--Display from most popular paint color to least
SELECT pa.colorname, 
...
--Display sales of each paint color by store   
SELECT s.storename, pa.colorname, 
       COUNT(p.purchaseid) AS NumberPurchased
    FROM purchase p
    INNER JOIN bicycle b
    ON p.bicycleserialnumber = b.serialnumber
    ...
    INNER JOIN store s 
    ON p.storeid = s.storeid
    GROUP BY CUBE(s.storename, pa.colorname)
    ORDER BY COUNT(p.purchaseid) DESC;


--Display part manufacturers that are most popular by store
